...
class ApiClient(object):
    ...
    def call_authentication_header(self, method, header_params, body, request_target=None, isResponseMLEforApi=False):
        ...
        if self.mconfig.authentication_type.upper() == GlobalLabelParameters.HTTP.upper():
            ...
            token = "Bearer " + token
            header_params['Authorization'] = str(token)
        header_params['v-c-sdk-telemetry-merchant-id'] = str(self.mconfig.merchant_id)
        if self.mconfig.isSDK:
            header_params['v-c-sdk-telemetry-mcp'] = "true"
    ...
    def _decrypt_mle_response(self, response_data_or_body):
        """
        Decrypt MLE encrypted response if MLE is configured and response is encrypted.
        
        :param response_data_or_body: Either response data string or response body string
        :return: Decrypted data if encrypted, otherwise original data
        :raises ApiException: If decryption fails
        """
        if not hasattr(self, 'mconfig'):
            return response_data_or_body
            
        try:
            if MLEUtility.check_is_mle_encrypted_response(response_data_or_body):
                decrypted_data = MLEUtility.decrypt_mle_response_payload(self.mconfig, response_data_or_body)
                return decrypted_data
        except Exception as e:
            if (hasattr(self, "logger") and
                hasattr(self.mconfig, "log_config") and
                self.mconfig.log_config.enable_log):
                self.logger.error(f"[MLE] Response decryption unsuccessful: {e}")
            raise ApiException(status=0, reason=f"MLE Response decryption unsuccessful: {e}")
    ...
    def __call_api(self, resource_path, method,
        ...
        try:
            ...
            # Handle MLE decryption for error responses (non-2xx status codes)
            if hasattr(api_exception, 'body'):
                api_exception.body = self._decrypt_mle_response(api_exception.body)
            raise

        # Handle MLE decryption for successful responses (2xx status codes)
        response_data.data = self._decrypt_mle_response(response_data.data)
